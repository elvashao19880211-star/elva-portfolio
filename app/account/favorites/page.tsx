...
const PRICES: Record<string, Record<string, string>> = {
  revival: { personal: '19.9', commercial: '399', source: '3999' },
  innovation: { commercial: '499', source: '3999' },
};
...
export default function FavoritesPage() {
  ...
  // 创新纹样不允许个人档
  const hasInnovation = selectedItems.some((item) => item.type === 'innovation');
  const effectiveTier = hasInnovation ? 'commercial' : tier;
  const showPersonalOption = !hasInnovation;

  const totalPrice = useMemo(() => {
    const raw = selectedItems.reduce((sum, item) => {
      const prices = PRICES[item.type] || PRICES.innovation;
      return sum + parseFloat(prices[effectiveTier] || prices.commercial);
    }, 0);
    return Math.round(raw * 10) / 10;
  }, [selectedItems, effectiveTier]);
  ...
  const handleConfirmPaid = () => {
    ...
    selectedItems.forEach((item) => {
      ...
      addPurchase({
        ...
        type: item.type,
        effectiveTier,
        price: `¥${prices[effectiveTier] || prices.commercial}`,
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-gold">
                        ¥{PRICES[item.type]?.[effectiveTier] || PRICES.innovation[effectiveTier] || PRICES.innovation.commercial}
                      </p>
                    </div>
